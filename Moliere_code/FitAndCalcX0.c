...
double GetTheta(TString filename, double data_percentage, double xl, double xr, double yb, double yt){
	...
	if(file->IsOpen()){
		...
		for(int i = 0; i <tsize; i++){
			...
			for (int j = 0; j < kinkx->size(); j++)
			{
				if (x->at(j) >= xl && x->at(j) < xr && y->at(j) >= yb && y->at(j) < yt){			
					xkink_h->Fill(kinkx->at(j)-0.000436); // extracted mean value
		...
		kink_plotX->cd();
		xkink_h->Draw();
		xkink_h->SetLineWidth(2);
		kink_plotY->cd();
		xkink_h->Draw();

		
		double xq[2]={0.07,0.93};
		double yq[2] ={0.,0.};
		double meanx = 0;
		double meany = 0;
		...
		llx->SetLineColor(kGreen+2);
		lrx->SetLineColor(kGreen+2);
		llx->SetLineWidth(2);
		lrx->SetLineWidth(2);
		kink_plotX->cd();		
		llx->Draw();
		...
		TF1 *f1 = new TF1("f1","[1]/((x-[0])*(x-[0])*(x-[0]))+[2]",-0.03,yq[0]);
		TF1 *f2 = new TF1("f2","gaus",yq[0],yq[1]);
		TF1 *f3 = new TF1("f3","[1]/((x-[0])*(x-[0])*(x-[0]))+[2]",yq[1],0.03);
		TF1 *total = new TF1("tot","f1(0)+f2(3)+f3(6)",-0.03,0.03);
		xkink_h->Fit("f1","0R");
		xkink_h->Fit("f2","0R+");
		xkink_h->Fit("f3","0R+");	
		f1->Draw("same");
		f2->SetRange(1.5*yq[0],1.5*yq[1]);		
		f2->Draw("same");
		f3->Draw("same");
		...
		double sigmax = 0;//gausf_x->GetParameter(2);
